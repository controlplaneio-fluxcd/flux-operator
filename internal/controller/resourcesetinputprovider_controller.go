...
func (r *ResourceSetInputProviderReconciler) restoreSkippedGitProviderResults(results *[]gitprovider.Result, obj *fluxcdv1.ResourceSetInputProvider) error {
	...
	if err != nil {
		return fmt.Errorf("invalid exportedInputs values: %w", err)
	}
	...
	for i, result := range *results {
		...
		if isSkipped {
			...
			// when the result is newly added
			if exportedInput == nil {
				continue
			}

			err := (*results)[i].OverrideFromExportedInputs(exportedInput)
			if err != nil {
				return fmt.Errorf("failed to override result from exportedInput: %w", err)
			}
		}
	}
...
func (r *ResourceSetInputProviderReconciler) callProvider(ctx context.Context,
	...
	if len(results) > 0 {
		err := r.restoreSkippedGitProviderResults(&results, obj)
