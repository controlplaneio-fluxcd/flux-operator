...
func TestArtifactsAttestation_Validate(t *testing.T) {
	...
	t.Run("fails when IssuedAt is in the future", func(t *testing.T) {
		g := NewWithT(t)
		testAtt := testArtifactsAttestation()
		testAtt.att.IssuedAt = time.Now().Add(time.Hour).Unix()
		aa := &testAtt
	...
	t.Run("allows IssuedAt within future tolerance", func(t *testing.T) {
		g := NewWithT(t)
		testAtt := testArtifactsAttestation()
		testAtt.att.IssuedAt = time.Now().Add(30 * time.Second).Unix()
		aa := &testAtt
	...
	t.Run("fails when Subject is not 'artifacts'", func(t *testing.T) {
		g := NewWithT(t)
		testAtt := testArtifactsAttestation()
		testAtt.att.Subject = "wrong-subject"
		aa := &testAtt
