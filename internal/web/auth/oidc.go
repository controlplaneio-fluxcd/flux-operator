...
// provider creates the oidc.Provider.
func (o *oidcProvider) provider(ctx context.Context) (*oidc.Provider, error) {
...
// verifyAccessToken implements oauth2Provider.
func (o *oidcProvider) verifyAccessToken(ctx context.Context, accessToken string) (string, []string, error) {
	p, err := o.provider(ctx)
	if err != nil {
		return "", nil, err
	}

	v := p.VerifierContext(ctx, &oidc.Config{ClientID: o.conf.Authentication.OAuth2.ClientID})
...
// verifyToken implements oauth2Provider.
func (o *oidcProvider) verifyToken(ctx context.Context, token *oauth2.Token) (string, []string, *authStorage, error) {
	idToken, ok := token.Extra("id_token").(string)
	if !ok {
		return "", nil, nil, fmt.Errorf("no id_token found in token response")
	}
	username, groups, err := o.verifyAccessToken(ctx, idToken)
	if err != nil {
		return "", nil, nil, err
	}
	as := &authStorage{
...
func (o *oidcProvider) config(ctx context.Context) (*oauth2.Config, error) {
	p, err := o.provider(ctx)
	if err != nil {
		return nil, err
	}
