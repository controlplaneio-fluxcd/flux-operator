...
func TestPermuter_NormalizesObjectNames(t *testing.T) {
	for _, tt := range []struct {
		name         string
		expectedName string
	}{
		{name: "My-ResourceSet", expectedName: "my_resourceset"},
		{name: "My_ResourceSet", expectedName: "my_resourceset"},
		{name: "My.ResourceSet", expectedName: "myresourceset"},
		{name: "My ResourceSet", expectedName: "myresourceset"},
		{name: "My@ResourceSet", expectedName: "myresourceset"},
		{name: "My#ResourceSet", expectedName: "myresourceset"},
		{name: "_a---b_-_", expectedName: "a_b"},
		{name: "A0..B", expectedName: "a0b"},
		{name: "A  B", expectedName: "ab"},
		{name: "A@@B", expectedName: "ab"},
		{name: "A##B", expectedName: "ab"},
	} {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := p.AddProvider(tt.name, []map[string]any{{"key": "value"}})
			g.Expect(err).NotTo(HaveOccurred())

			perms := p.Combine()
			g.Expect(perms).To(HaveLen(1))
			g.Expect(perms[0]).To(HaveKey(tt.expectedName))
		})
	}
}
