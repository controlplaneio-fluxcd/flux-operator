...
func AddProviderReference(input map[string]any, provider fluxcdv1.InputProvider) {
	...
	input["providerAPIVersion"] = providerType.GroupVersion().String()
	input["providerKind"] = providerType.Kind
	input["providerName"] = provider.GetName()
	input["providerNamespace"] = provider.GetNamespace()
}
