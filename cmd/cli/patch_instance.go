...
fetching CRDs from the main branch and using 'preview' image tags.
...
func patchInstanceCmdRun(cmd *cobra.Command, args []string) error {
	...
	// Remove previously generated patches (image and CRD).
	if instance.Spec.Kustomize != nil {
		var removed int
		data, removed = removeGeneratedPatches(data, instance.Spec.Kustomize.Patches)
		if removed > 0 {
...
func computeAllPatches(
	...
	for _, component := range components {
		...
		imgPatch, err := computeImagePatch(component, fromMinor, target, registry)
		if err != nil {
...
// isGeneratedPatch returns true if the patch targets a CRD or is a
// Deployment image replacement (i.e. was generated by this command).
func isGeneratedPatch(p kustomize.Patch) bool {
	if p.Target == nil {
		return false
	}
...
// from the raw YAML, preserving all other content and formatting.
func removeGeneratedPatches(data []byte, patches []kustomize.Patch) ([]byte, int) {
	if len(patches) == 0 {
		return data, 0
	}
...
func buildCRDURLs(
	...
	major, err := version.ControllerMajor(controller)
	if err != nil {
		return "", "", err
	}

	ctrlFromMinor, err := version.ControllerMinorForFluxMinor(controller, fromMinor)
	if err != nil {
		return "", "", err
	}
	...
	if target.isMain {
		...
		ctrlToMinor, err := version.ControllerMinorForFluxMinor(controller, target.fluxMinor)
		if err != nil {
			return "", "", err
		}
