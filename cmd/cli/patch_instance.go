...
credentials stored by 'gh auth login'.`,
  ...
  # Remove generated patches for a controller, reverting it to the distribution version
  flux-operator patch instance -f instance.yaml -c image-reflector-controller --rm
...
func init() {
	...
	patchInstanceCmd.Flags().BoolVar(&patchInstanceArgs.remove, "rm", false,
		"Remove previously generated patches for the selected components instead of adding new ones. "+
			"The --version and --registry flags are ignored in this mode.")
...
func patchInstanceCmdRun(cmd *cobra.Command, args []string) error {
	...
	// Determine which components to operate on.
	components := instance.GetComponents()
	if len(patchInstanceArgs.components) > 0 {
		components = patchInstanceArgs.components
	}

	// Remove-only mode reverts the components to .spec.distribution.version by
	// wiping the patches this command generated for them.
	if patchInstanceArgs.remove {
		return patchInstanceRemove(data, path, &instance, components)
	}
	...
	if err := writeOutput(out, path, fmt.Sprintf("Appended %d patches to %s", len(patches), path)); err != nil {
		return err
	}
	return validatePatchedFile(path)
}

// patchInstanceRemove wipes the patches generated by this command for the given
// components from the manifest and writes the result back.
func patchInstanceRemove(data []byte, path string, instance *fluxcdv1.FluxInstance, components []string) error {
	if instance.Spec.Kustomize == nil || len(instance.Spec.Kustomize.Patches) == 0 {
		rootCmd.PrintErrln(`✔`, "No patches to remove")
		return nil
	}
	out, removed := removeGeneratedPatches(data, instance.Spec.Kustomize.Patches, components)
	if removed == 0 {
		rootCmd.PrintErrln(`✔`, "No generated patches found for the selected components")
		return nil
	}
	if err := writeOutput(out, path, fmt.Sprintf("Removed %d patches from %s", removed, path)); err != nil {
		return err
	}
	return validatePatchedFile(path)
}

// validatePatchedFile runs 'build instance' on the patched file unless the
// input came from stdin.
func validatePatchedFile(path string) error {
	if patchInstanceArgs.filename == "-" {
		return nil
	}
...
// writeOutput writes the patched YAML to the original file or stdout, printing
// doneMsg after a successful file write.
func writeOutput(out []byte, path, doneMsg string) error {
	if patchInstanceArgs.filename == "-" {
...
// isGeneratedPatchForComponents reports whether the patch was generated by this
// command for one of the given components. It matches on both the target and the
// patch body so user-authored patches sharing a target (e.g. extra args on a
// controller Deployment) are never touched.
func isGeneratedPatchForComponents(p kustomize.Patch, components []string) bool {
	t := p.Target
	// The command only ever sets Kind and Name; anything narrower is the user's.
	if t == nil || t.Group != "" || t.Version != "" || t.Namespace != "" ||
		t.AnnotationSelector != "" || t.LabelSelector != "" {
		return false
	}
	switch t.Kind {
	case "Deployment":
		return slices.Contains(components, t.Name) && isGeneratedImagePatch(p.Patch)
	case "CustomResourceDefinition":
		return crdBelongsToComponents(t.Name, components) && isJSON6902(p.Patch)
	}
	return false
}

// isGeneratedImagePatch reports whether body is exactly the single image
// replace operation that computeImagePatch emits.
func isGeneratedImagePatch(body string) bool {
	ops, ok := parseJSON6902(body)
	if !ok || len(ops) != 1 {
		return false
	}
	return ops[0]["op"] == "replace" && ops[0]["path"] == "/spec/template/spec/containers/0/image"
}

// isJSON6902 reports whether body is an inline RFC 6902 patch (an array of
// operation objects) rather than a user strategic-merge patch.
func isJSON6902(body string) bool {
	_, ok := parseJSON6902(body)
	return ok
}

// parseJSON6902 parses an inline RFC 6902 patch into its operation list,
// returning false for strategic-merge (map) or malformed patches.
func parseJSON6902(body string) ([]map[string]any, bool) {
	var ops []map[string]any
	if err := yaml.Unmarshal([]byte(body), &ops); err != nil || len(ops) == 0 {
		return nil, false
	}
	for _, op := range ops {
		if _, ok := op["op"]; !ok {
...
// crdBelongsToComponents reports whether the named CRD is owned by one of the components.
func crdBelongsToComponents(crdName string, components []string) bool {
	for _, c := range components {
		for _, kind := range fluxcdv1.ControllerCRDKinds[c] {
			gk, err := fluxcdv1.FluxGroupFor(kind)
			if err != nil {
				continue
			}
