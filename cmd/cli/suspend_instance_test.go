...
func TestSuspendInstanceCmd(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		expectError bool
	}{
		{
			...
			args: []string{"suspend", "instance", "flux"},
		},
		{
			name:        "missing name",
			args:        []string{"suspend", "instance"},
			expectError: true,
		},
		{
			name:        "non-existent instance",
			args:        []string{"suspend", "instance", "nonexistent"},
			expectError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ns, err := testEnv.CreateNamespace(ctx, "test-suspend-inst")
