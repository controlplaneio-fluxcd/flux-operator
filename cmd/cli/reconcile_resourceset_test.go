...
func TestReconcileResourceSetCmd(t *testing.T) {
	tests := []struct {
		name        string
		args        []string
		wait        bool
		...
		{
			name: "reconcile resourceset without wait",
			args: []string{"reconcile", "resourceset", "test-rs", "--wait=false"},
		},
		{
			name: "reconcile resourceset with wait",
			args: []string{"reconcile", "resourceset", "test-rs", "--wait"},
			wait: true,
		},
		{
			name: "reconcile using alias",
			args: []string{"reconcile", "rset", "test-rs", "--wait=false"},
		},
		{
			name:        "missing name",
			args:        []string{"reconcile", "resourceset"},
			expectError: true,
		},
		{
			name:        "non-existent resourceset",
			args:        []string{"reconcile", "resourceset", "nonexistent", "--wait=false"},
			expectError: true,
		},
	}
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			ns, err := testEnv.CreateNamespace(ctx, "test-reconcile-rs")
