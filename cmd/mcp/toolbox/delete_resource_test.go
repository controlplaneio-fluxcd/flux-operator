...
func TestManager_HandleDeleteKubernetesResource(t *testing.T) {
	...
	for _, test := range tests {
		t.Run(test.testName, func(t *testing.T) {
			...
			var input deleteKubernetesResourceInput
			err := json.Unmarshal(request.Params.Arguments, &input)
			g.Expect(err).ToNot(HaveOccurred())
			result, content, err := m.HandleDeleteKubernetesResource(context.Background(), request, input)
			g.Expect(err).ToNot(HaveOccurred())
