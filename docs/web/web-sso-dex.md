...
[Dex](https://dexidp.io/) is an open-source OIDC provider that can federate multiple identity sources
such as LDAP, SAML, and social logins (GitHub, GitLab, Gitea, Google, LinkedIn, etc.).

This guide will walk you through the steps to set up Dex as a standalone OIDC provider with a static user
directory and configure the Flux Web UI to use Dex for authentication.
