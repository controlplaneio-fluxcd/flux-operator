...
When merging the input sets from each object, the `Permute` strategy places each input set
under a key that is the normalized name of the object providing the input set. The normalization
process is as follows:

1. Uppercase letters are converted to lowercase.
2. Hyphens (`-`) are converted to underscores (`_`).
3. All non-alphanumeric characters (except underscores) are removed.
