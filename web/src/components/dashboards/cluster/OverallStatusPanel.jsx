...
export function OverallStatusPanel({ report }) {
  ...
  // Failures make the whole card a link to the filtered resources list.
    ...
    <>
      ...
      {statusInfo.status === 'initializing' && (
        <svg class={`w-10 h-10 ${statusInfo.color} animate-spin`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
          <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="m20 4v5h-0.582m-15.356 2a8.001 8.001 0 0 1 15.356-2m0 0h-4.418m-11 11v-5h0.581m0 0a8.003 8.003 0 0 0 15.357-2m-15.357 2h4.419" />
        </svg>
      )}
    </>
  )
  ...
  return (
    <StatusHeroCard
      ...
      icon={icon}
      href={hasFailures ? '/resources?status=Failed' : undefined}
      lastUpdatedAt={reportUpdatedAt.value}
    >
